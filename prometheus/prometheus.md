...
blackbox-exporter安装
    ...
    --config.file=/config/blackbox.yml 指定的配置文件为 prometheus/exporter/blackbox_exporter/blackbox.yml
...
```
所有配置文件都已经集成好了 
redis mysql nginx 需要在 prometheus/prometheus/prometheus.yml 更改对应exproter对应的地址

node监控节点可能会比较多 使用文件发现的配置需要在 prometheus/prometheus/nodes/nodes.yaml 下配置
```
更改mysql redis nginx的配置  prometheus/prometheus/prometheus.yml
![Image text](../src/img/prometheus-config1.png) 

配置连接node节点的连接配置 prometheus/prometheus/nodes/nodes.yaml 
![Image text](../src/img/prometheus-nodes-config.png) 


#### 报警配置
```
1.运行alert.py  python2.7 alert.py 将监听39092 端口
2.在 prometheus/alertmanager/config.yml 中配置webhook_configs接口地址如 http://10.1.34.13:39092/send
...
```
如图:
![Image text](../src/img/prometheus-grafana-config.png) 
